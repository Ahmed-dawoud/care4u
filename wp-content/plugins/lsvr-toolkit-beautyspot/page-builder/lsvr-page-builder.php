...
----------------------------------------------------------------------------- */
    ...
    function lsvr_page_builder_assets() {

        global $theme_version;

        // jHtmlArea WYSIWYG editor
        // http://jhtmlarea.codeplex.com/
        wp_register_style( 'htmlarea', plugins_url( 'lsvr-toolkit-beautyspot' ) . '/library/htmlarea/jHtmlArea.css', false, $theme_version );
        wp_enqueue_style( 'htmlarea' );
        wp_register_style( 'htmlarea-colorpicker', plugins_url( 'lsvr-toolkit-beautyspot' ) . '/library/htmlarea/jHtmlArea.ColorPickerMenu.css', false, $theme_version );
        wp_enqueue_style( 'htmlarea-colorpicker' );
        wp_register_script( 'htmlarea', plugins_url( 'lsvr-toolkit-beautyspot' ) . '/library/htmlarea/jHtmlArea.0.7.5.js', array('jquery'), $theme_version );
        wp_enqueue_script( 'htmlarea' );
        wp_register_script( 'htmlarea-colorpicker', plugins_url( 'lsvr-toolkit-beautyspot' ) . '/library/htmlarea/jHtmlArea.colorpickermenu.0.7.0.min.js', array('jquery'), $theme_version );
        wp_enqueue_script( 'htmlarea-colorpicker' );

        // Custom CSS & JS
        wp_register_style( 'lsvr-page-builder', plugins_url( 'lsvr-toolkit-beautyspot' ) . '/library/css/page-builder.css', false, $theme_version );
        wp_enqueue_style( 'lsvr-page-builder' );
        wp_register_script( 'lsvr-page-builder', plugins_url( 'lsvr-toolkit-beautyspot' ) . '/library/js/page-builder.js', array('jquery'), $theme_version, true );
        wp_enqueue_script( 'lsvr-page-builder' );

    }
    if ( strstr( $_SERVER['REQUEST_URI'], 'wp-admin/themes.php?page=aq-page-builder' ) ) {
        add_action( 'admin_enqueue_scripts', 'lsvr_page_builder_assets' );
    }
