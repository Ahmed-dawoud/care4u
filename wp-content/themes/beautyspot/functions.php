...
----------------------------------------------------------------------------- */
	...
	// INCLUDE
	if ( ! class_exists( 'ReduxFramework' ) && file_exists( dirname( __FILE__ ) . '/includes/redux/ReduxCore/framework.php' ) ) {
		require_once( dirname( __FILE__ ) . '/includes/redux/ReduxCore/framework.php' );
	}
	...
	if ( ! function_exists( 'lsvr_register_sidebars' ) ) {
		function lsvr_register_sidebars() {
			...
			register_sidebar( array(
				...
				'class'         => '',
				'before_widget' => '<li id="%1$s" class="widget col-md-6 %2$s">',
				'after_widget'  => '</li>',
				'before_title'  => '<h3 class="widget-title m-secondary-font">',
				'after_title'   => '</h3>'
			));
...
----------------------------------------------------------------------------- */
		...
		function lsvr_load_theme_styles(){
			...
			if ( lsvr_get_field( 'enable_google_fonts', true, true ) ) {
				if ( is_array( lsvr_get_field( 'primary_font' ) ) ) {
					...
					wp_add_inline_style( 'main-style', 'body, input, textarea, select { font-family: \'' . $primary_font_family . '\', Arial, sans-serif; font-size: ' . $primary_font_size . '; font-weight: ' . $primary_font_weight . '; } .wpcf7-list-item-label { font-family: \'' . $primary_font_family . '\', Arial, sans-serif; }' );
				}
				if ( is_array( lsvr_get_field( 'secondary_font' ) ) ) {
					...
					$secondary_font_elements = '.m-secondary-font, .header-menu > ul > li > span, .various-content h1, .various-content h2, .various-content h3, .various-content h4, .various-content h5, ';
					...
					wp_add_inline_style( 'main-style', $secondary_font_elements . ' { font-family: ' . $secondary_font_family . '; }' );
				}
			}
	...
	if ( ! function_exists( 'lsvr_load_theme_scripts' ) ) {
		function lsvr_load_theme_scripts() {
			...
			$html5shim = create_function( '', 'echo \'<!--[if lt IE 9]><script src="' . get_template_directory_uri() . '/library/js/respond.min.js"></script><![endif]-->\';' );
			add_action( 'wp_head', $html5shim );
    ...
    */
	...
	if ( ! function_exists( 'lsvr_dummy_wp_get_attachment_url' ) ) {
		function lsvr_dummy_wp_get_attachment_url( $url, $post_id ){
			return 'http://demos.volovar.net/beautyspot.wp/files/dummy.jpg';
		}
	}
