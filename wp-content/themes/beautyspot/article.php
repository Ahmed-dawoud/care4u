...
<?php else : ?>
    ...
    <article <?php post_class(); ?>>
		...
		<?php else: ?>
			...
			<?php $post_custom_keys = get_post_custom_keys();
			$embed_code = false;
			if ( $post_custom_keys ) {
				$i = 1;
				foreach ( $post_custom_keys as $key ) {
					if ( strpos( $key, 'oembed' ) ) {
						foreach ( get_post_custom_values( $key ) as $embed ){
							if ( $i == 1 ) { $embed_code = $embed; } $i++;
						}
					}
				}
			} ?>

			<?php if ( $embed_code && has_post_format( 'video' ) ) : ?>
			<!-- ARTICLE VIDEO : begin -->
			...
			<?php elseif ( $embed_code && has_post_format( 'audio' ) ) : ?>
			<!-- ARTICLE AUDIO : begin -->
